...
const MyAppointment = () => {
  ...
  return (
    <div>
      ...
      <p className='pb-3 mt-10 font-medium text-slate-900 bordeer-b'>My Appointments</p>
          ...
          <div key={index} className=" grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b">
            ...
            <div className=" flex flex-col gap-2 justify-end">
              ...
              {!item.cancelled && !item.isCompleted && <button onClick={() => cancelAppointment(item._id)} className='text-sm text-stone-500 text-center sm:min-w-48 py-2 border rounded hover:bg-red-600 hover:text-white transition-all duration-300'>Cancel Appointment</button>}
               ...
               {item.cancelled && !item.isCompleted &&  <button className='text-sm border border-red-500 py-2 text-red-700 sm:min-w-48'>Appointment cancelled</button>}
